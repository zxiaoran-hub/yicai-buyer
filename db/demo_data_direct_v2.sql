-- ============================================
-- 异采 YiCai 演示数据 - 直接插入版 v2
-- 修复：数组列使用 PostgreSQL 标准语法
-- ============================================
...
-- ========== 3. 插入供应商 ==========
-- 注意：category 列如果是 TEXT[] 数组类型，用 '{"护肤"}' 语法
-- certifications 列同理
...
('f7478991-2b89-45b7-aae8-62f58b8ffe35', (SELECT id FROM suppliers WHERE user_id='cce118c5-78f5-4977-9822-87fe74085b1d'), (SELECT id FROM companies WHERE name='星辰美妆（演示）'), 'cooperating', '{"核心供应商","面膜"}', '长期合作，品质稳定', '平台推荐'),
('f7478991-2b89-45b7-aae8-62f58b8ffe35', (SELECT id FROM suppliers WHERE user_id='b56016f2-4daa-4dbd-a14c-a76bb94bb644'), (SELECT id FROM companies WHERE name='星辰美妆（演示）'), 'contacted', '{"彩妆","新供应商"}', '已沟通，待打样', '展会接触'),
('f7478991-2b89-45b7-aae8-62f58b8ffe35', (SELECT id FROM suppliers WHERE user_id='0bd203b0-a44b-4cec-a49d-e0a377c81034'), (SELECT id FROM companies WHERE name='星辰美妆（演示）'), 'potential', '{"精华","备选"}', '正在了解中', '平台搜索')
ON CONFLICT DO NOTHING;
